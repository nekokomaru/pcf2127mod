...
//!! MSF interrupt
#define PCF2127_BIT_WD_CTL_TITP			BIT(5)	// MSF interrupt type
#define PCF2127_BIT_CTRL2_MSF                   BIT(7)	// MSF interrupt flag
#define PCF2127_REG_CLKOUT_CTL		0x0F		// CLKOUT ctl
#define PCF2127_REG_AGING_OFFSET	0x19		// AGING offset

//!! gloval value for module parameter
// alarm enable
...
static int pcf2127_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	alrm->enabled = ((ctrl2 & PCF2127_BIT_CTRL2_AIE) != 0) ? 1 : 0;
	alrm->pending = ((ctrl2 & PCF2127_BIT_CTRL2_AF) != 0) ? 1 : 0;
...
//!! make /sys/class/rtc/rtcx/alarmwaked interface
...
//!! make /sys/class/rtc/rtcx/batterylow interface
...
static struct attribute *pcf2127_attrs[] = {
	...
	&dev_attr_alarmwaked.attr,
	&dev_attr_batterylow.attr,
	NULL
};
...
static int pcf2127_clear_int(struct device *dev, struct regmap *map)
{
	...
	data = 0;
        ...
        if((data & PCF2127_BIT_CTRL1_TSF1) != 0 ) {
                dev_info(dev, "TSF1 is HI\n");
                // clear timestamp interrupt 1
                data &= ~PCF2127_BIT_CTRL1_TSF1;
                dev_info(dev, "Clear TSF1\n");
        }
...
static int pcf2127_init_regs(struct device *dev, struct regmap *map)
{
	...
	// power management is standard mode, enable low detection
	data = 0;
	...
	data = 0;
        ret = regmap_read(map, PCF2127_REG_CLKOUT_CTL, &data);
        if (ret) {
        ...
        ret = regmap_write(map, PCF2127_REG_CLKOUT_CTL, data);
        if (ret) {
	...
	// aging is default = 0b1000
        data = 0;
        ret = regmap_read(map, PCF2127_REG_AGING_OFFSET, &data);
        if (ret) {
	...
	data |= 0x08;
        ...
        ret = regmap_write(map, PCF2127_REG_AGING_OFFSET, data);
        if (ret) {
				...
				const struct i2c_device_id *id)
{
	...
	// so, regmap_bulk_* functions require a buffer of unsigned chars.
...
{
	int ret = 0;
	ret = i2c_add_driver(&pcf2127_i2c_driver);

	//!! put i2c device
	if((ret == 0) && (i2c_name != NULL) && (slvaddr >= 0) && (bus_no >= 0)) {
                // make i2c board info
